...
import { audioLabel, normalizeProgramText, programGenreClass, programGenreLabels, programStatus } from "./program";
...
describe("programStatus", () => {
  it("reports upcoming, ongoing and finished programs", () => {
    ...
    expect(programStatus(program({ startAt: 2000, duration: 100 }))).toBe("放送予定");
    expect(programStatus(program({ startAt: 500, duration: 1000 }))).toBe("放送中");
    expect(programStatus(program({ startAt: 0, duration: 100 }))).toBe("放送終了");
    vi.useRealTimers();
  });
});
...
describe("programGenreLabels", () => {
  it("labels a known lv1/lv2 pair", () => {
    expect(programGenreLabels(program({ genres: [{ lv1: 0, lv2: 1 }] }))).toEqual(["ニュース／報道 / 天気"]);
  });

  it("falls back to lv1-only label when lv2 is unknown", () => {
    expect(programGenreLabels(program({ genres: [{ lv1: 0 }] }))).toEqual(["ニュース／報道"]);
  });

  it("labels lv1 15 as other", () => {
    expect(programGenreLabels(program({ genres: [{ lv1: 15 }] }))).toEqual(["その他"]);
  });
});
...
describe("programGenreClass", () => {
  ...
  it("falls back to genre-other when missing or out of range", () => {
    ...
    expect(programGenreClass(program({ genres: [{ lv1: 15 }] }))).toBe("genre-other");
  });
});

describe("audioLabel", () => {
  it("joins available audio fields", () => {
    const label = audioLabel({ componentType: 1, componentTag: 2, samplingRate: 48000, langs: ["jpn"] });
