...
export default function Logs() {
  ...
  const eventAutoScroll = useAutoScroll<HTMLDivElement>([
    events.data,
    liveEvents,
  ])
  ...
  const allEvents = useMemo(() => {
    ...
    return combined.sort((a, b) => a.time - b.time).slice(-500)
  ...
  return (
    <PageFrame
      title="ログ"
      subtitle="APIイベントとサーバーログをリアルタイムに表示します。"
    >
      <ErrorList
        errors={[events.error, log.error, eventStreamError, logStreamError]}
      />
        ...
        <Panel
          ...
          action={
            <LogPanelActions
              ...
              following={eventAutoScroll.following}
              onFollow={eventAutoScroll.scrollToBottom}
            />
          }
        >
          <EventList
            events={allEvents}
            onScroll={eventAutoScroll.onScroll}
            scrollRef={eventAutoScroll.ref}
          />
        </Panel>
        <Panel
          title="ログ"
          action={
            <LogPanelActions
              connected={logStreamConnected}
              following={logAutoScroll.following}
              onFollow={logAutoScroll.scrollToBottom}
            />
          }
        >
          <pre
            className="log-view"
            onScroll={logAutoScroll.onScroll}
            ref={logAutoScroll.ref}
          >
            {(log.data ?? '') + liveLog || 'ログはありません。'}
          </pre>
        </Panel>
      </div>
    </PageFrame>
