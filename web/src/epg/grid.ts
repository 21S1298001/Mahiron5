...
const sharedProgramResolverCache = new WeakMap<
  Map<string, Program[]>,
  (program: Program) => string | undefined
>()

export function makeEpgColumns(
  services: Service[],
  ...
  for (const [groupKey, groupServices] of groups) {
    ...
    for (const [index, service] of groupServices.entries()) {
      const programs =
        programsByService.get(`${service.networkId}:${service.serviceId}`) ?? []
      const contentEntries = programs.map((program) => ({
        key: programContentKey(program, resolveSharedProgramKey),
        sharedWithFoldedService: isSharedWithServiceSet(
          program,
          foldedServiceKeys,
        ),
      }))
      const canSplit =
        isStableEpgService(service) &&
        hasReadyBaseline &&
        contentEntries.length > 0
      const hasDistinctContent =
        canSplit &&
        contentEntries.some(
          ({ key, sharedWithFoldedService }) =>
...
export function makeEpgProgramBlocks(
  ...
  const uniquePrograms = new Map<
    string,
    { program: Program; service: Service }
  >()
  const resolveSharedProgramKey = sharedProgramKeyResolverFor(programsByService)
  ...
  return [...uniquePrograms.values()]
    .sort(
      (a, b) =>
        a.program.startAt - b.program.startAt || a.program.id - b.program.id,
    )
    .map(({ program, service }) => ({
...
export function programContentKey(
  program: Program,
  resolveSharedProgramKey?: (program: Program) => string | undefined,
) {
...
export function makeSharedProgramKeyResolver(programs: Program[]) {
  ...
  for (const program of programs) {
    const source = programEndpointKey(
      program.networkId,
      program.serviceId,
      program.eventId,
    )
...
function sharedProgramKeyResolverFor(
  programsByService: Map<string, Program[]>,
) {
  ...
  const resolver = makeSharedProgramKeyResolver(
    flattenProgramGroups(programsByService),
  )
...
function isSharedWithServiceSet(program: Program, serviceKeys: Set<string>) {
  return (program.relatedItems ?? []).some(
    (item) =>
      item.type === 'shared' &&
      item.serviceId != null &&
      serviceKeys.has(
